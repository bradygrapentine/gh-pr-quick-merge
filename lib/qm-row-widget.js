...
(function attach(scope) {
  ...
  const METHOD_ORDER = ["squash", "merge", "rebase"];
  ...
  function makeRowWidget(opts) {
    ...
    const { pr } = opts;
    const getDefault = typeof opts.getDefaultMethod === "function" ? opts.getDefaultMethod : () => "squash";
    let activeMethod = getDefault() || "squash";
    if (!METHOD_ORDER.includes(activeMethod)) activeMethod = "squash";
    ...
    caretBtn.innerHTML = '<svg width="10" height="10" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M3 6l5 5 5-5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
