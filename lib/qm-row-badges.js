...
(function attach(scope) {
  ...
  function _ensureBadgeStrip(container) {
    ...
    // Preferred mount: as the FIRST CHILD of the title anchor itself.
    // Putting the strip inside the link guarantees it sits on the
    // title's baseline, regardless of how GitHub's title row is
    // flexed. Trade-off: the badges are inside the clickable target,
    // so we stopPropagation on badge clicks (handled at element
    // construction below).
    const titleLink = row && (
    ...
    if (titleLink) {
      ...
      titleLink.insertBefore(strip, titleLink.firstChild);
    } else {
  ...
  function buildCommentsBadge(state, pr) {
    ...
    // Render as a span (not <a>) — the strip mounts INSIDE the row's
    // title <a>, and nested <a> is invalid HTML that browsers
    // auto-close, breaking the title layout. The role + click handler
    // give us the jump-to-comments behaviour without an actual anchor.
    const el = document.createElement("span");
    el.className = "qm-row-badge qm-row-badge-comments";
    el.setAttribute("role", "link");
    el.setAttribute("tabindex", "0");
    el.dataset.qmCommentsHref = `/${pr.owner}/${pr.repo}/pull/${pr.num}#issue-comment-area`;
    el.textContent = `💬 ${count}`;
    el.setAttribute("aria-label", `${count} comment${count === 1 ? "" : "s"} — jump to comments`);
    const navigate = (ev) => {
      ev.preventDefault();
      ev.stopPropagation();
      window.location.assign(el.dataset.qmCommentsHref);
    };
    el.addEventListener("click", navigate);
    el.addEventListener("keydown", (ev) => {
      if (ev.key === "Enter" || ev.key === " ") navigate(ev);
    });
    return el;
  }
  ...
  function applyRowBadges(container, state, pr, opts = {}) {
    ...
    // Comments badge removed (UI pass 6) — GitHub already surfaces a
    // comments count next to each row title; ours was redundant.
