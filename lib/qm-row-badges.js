...
(function attach(scope) {
  ...
  /** Build / return the badge container inside this row's `.qm-container`. */
  function _ensureBadgeStrip(container) {
    let strip = container.querySelector('[data-qm-badge="strip"]');
    if (strip) return strip;
    strip = document.createElement("span");
    strip.className = "qm-row-badge-strip";
    strip.setAttribute(BADGE_ATTR, "strip");
    // Mount before any merge buttons so badges read left-to-right.
