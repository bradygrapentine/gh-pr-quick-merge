...
(function attach(scope) {
  ...
  // Live adapters keyed by hostId. Adapters self-register at load time
  // by calling QM_HOSTS.register(adapter). content.js looks them up via
  // getAdapter(input) which combines detect() with this map.
  const _ADAPTERS = Object.create(null);

  /**
   * Register a HostAdapter. Throws if the adapter doesn't satisfy the
   * shape contract or its hostId isn't in REGISTRY.
   *
   * @param {HostAdapter} adapter
   * @returns {HostAdapter}
   */
  function register(adapter) {
    assertHostAdapterShape(adapter);
    if (!REGISTRY[adapter.hostId]) {
      throw new Error(`HostAdapter: unknown hostId "${adapter.hostId}" — add to REGISTRY first`);
    }
    _ADAPTERS[adapter.hostId] = adapter;
    return adapter;
  }

  /**
   * Resolve the active adapter for a URL or location-like input. Returns
   * null when no adapter is registered for the detected host (or when no
   * host matches at all).
   *
   * @param {string|{hostname:string}} input
  ...
  function getAdapter(input) {
    const hostId = detect(input);
    if (!hostId) return null;
    return _ADAPTERS[hostId] || null;
  }

  /** Test-only: drop all registered adapters. */
  function _resetAdaptersForTests() {
    for (const k of Object.keys(_ADAPTERS)) delete _ADAPTERS[k];
  }
