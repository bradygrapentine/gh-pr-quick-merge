...
(function attach(scope) {
  ...
  function decideMergeUi(state) {
    ...
    if (state.mergeable_state !== "clean") return "hide";
  ...
  function ensurePrPageActionBar(ctx) {
    ...
    // (5) Repo-default-aware merge: render only ONE of merge/squash
    // when the user has set a default for this repo, so the action bar
    // isn't redundant. Default-default = both visible.
    const preferredMethod = (ctx.repoDefault || "").toLowerCase();
    const showMerge = mergeMode === "show" && preferredMethod !== "squash";
    const showSquash = mergeMode === "show" && preferredMethod !== "merge";
